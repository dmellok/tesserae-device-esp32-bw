...
/* Battery sense. The generic ESP32 dev board has no fixed battery wiring,
 * so this is a convention you opt into: route the pack through a 1:2 (or
 * 1:3) resistor divider into an ADC1 pin and set the channel + divider to
 * match. Defaults: ADC1 channel 7 (= GPIO35 on the classic ESP32) and a
 * 1:2 divider. With nothing wired, the read floats and the heartbeat
 * reports an "unknown"-ish value -- harmless. */
...
static int read_battery_mv(void)
{
    ...
    /* The classic ESP32 only ships the line-fitting calibration scheme;
     * the S2/S3/C-series use curve fitting. Pick whichever the target
     * SoC actually provides so this file builds for both. */
                           ...
                           time_t sleep_until)
{
    ...
    int n = snprintf(dst, dst_sz,
             ...
             "\"ip\":\"%s\",\"fw_version\":\"%s\","
             "\"kind\":\"esp32_bw_client\",\"panel_w\":%d,\"panel_h\":%d,"
