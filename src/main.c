 ...
 * frame URL, downloads the panel-native 1bpp .bin, paints it, and goes
 * back to deep sleep.
 *
...
#  else
    /* The classic ESP32 has no USB-serial-JTAG peripheral (this board's
     * USB is a CP2102 UART bridge), so there's no SOF signal to sniff a
     * connected host from. Default to the battery path; define
     * DEV_DISABLE_SLEEP in secrets.h to loop while iterating with the
     * serial monitor open. */
#  endif
#endif
