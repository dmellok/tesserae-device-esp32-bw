...
static esp_err_t on_http(esp_http_client_event_t *e)
{
    ...
    case HTTP_EVENT_ON_DATA: {
        ...
        /* Grow geometrically; start at 16KB (one 15000-byte frame). */
        if (need > ctx->cap) {
            size_t new_cap = ctx->cap ? ctx->cap : 16384;
            while (new_cap < need) new_cap *= 2;
            if (new_cap > IMAGE_FETCH_MAX_BYTES) new_cap = IMAGE_FETCH_MAX_BYTES;

            /* Internal RAM (no PSRAM on this board). */
            uint8_t *grown = heap_caps_realloc(ctx->out->data, new_cap, MALLOC_CAP_DEFAULT);
            if (!grown) {
